...
static void simulate_one_cycle_with_forwarding_internal() {
    ...
    if (pipeline[WB].valid && !is_nop(pipeline[WB].instr)) {
        ...
        if (instr_writes_to_reg(pipeline[WB].instr)) {
            int dest_reg = get_dest_reg(pipeline[WB].instr);
            if (dest_reg != 0) { 
                state.registers[dest_reg] = pipeline[WB].result_val; 
                if(register_written) register_written[dest_reg] = 1;
            }
        }
    ...
    // Determines operands with forwarding, executes, sets pipeline[EX].result_val.
    // Sets pipeline[EX].branch_taken, and pipeline[EX].branch_target (for branches or eff_addr for LD/ST).
    pipeline[EX].branch_taken = 0; // Default for current EX instruction

    if (pipeline[EX].valid && !is_nop(pipeline[EX].instr)) {
        DecodedInstruction instr_ex = pipeline[EX].instr;
        int32_t val_rs = state.registers[instr_ex.rs]; 
        int32_t val_rt = state.registers[instr_ex.rt]; 
        int32_t ex_alu_output_value = 0;

        // ** FORWARDING LOGIC FOR OPERANDS TO CURRENT EX STAGE **
        // Rs operand:
        // Check MEM output (instruction that was in MEM last cycle, its result is available now)
        if (pipeline[MEM].valid && instr_writes_to_reg(pipeline[MEM].instr) && get_dest_reg(pipeline[MEM].instr) == instr_ex.rs && instr_ex.rs != 0) {
            val_rs = pipeline[MEM].result_val; // Forward from MEM stage output
        } 
        // Check WB output (instruction that was in WB last cycle, its result is available now)
        // This path is for instruction that was 2 stages ahead of current EX
        else if (pipeline[WB].valid && instr_writes_to_reg(pipeline[WB].instr) && get_dest_reg(pipeline[WB].instr) == instr_ex.rs && instr_ex.rs != 0) {
            val_rs = pipeline[WB].result_val; // Forward from WB stage output
        }

        // Rt operand (if it's a source)
        if (instr_ex.type == R_TYPE || instr_ex.opcode == BEQ || instr_ex.opcode == STW) {
            if (pipeline[MEM].valid && instr_writes_to_reg(pipeline[MEM].instr) && get_dest_reg(pipeline[MEM].instr) == instr_ex.rt && instr_ex.rt != 0) {
                val_rt = pipeline[MEM].result_val;
            } else if (pipeline[WB].valid && instr_writes_to_reg(pipeline[WB].instr) && get_dest_reg(pipeline[WB].instr) == instr_ex.rt && instr_ex.rt != 0) {
        ...
        switch (instr_ex.opcode) {
            // ALU R-Type
            case ADD:  ex_alu_output_value = val_rs + val_rt; break;
            case SUB:  ex_alu_output_value = val_rs - val_rt; break;
            case MUL:  ex_alu_output_value = val_rs * val_rt; break;
            case OR:   ex_alu_output_value = val_rs | val_rt; break;
            case AND:  ex_alu_output_value = val_rs & val_rt; break;
            case XOR:  ex_alu_output_value = val_rs ^ val_rt; break;
            // ALU I-Type
            case ADDI: ex_alu_output_value = val_rs + instr_ex.immediate; break;
            case SUBI: ex_alu_output_value = val_rs - instr_ex.immediate; break;
            case MULI: ex_alu_output_value = val_rs * instr_ex.immediate; break;
            case ORI:  ex_alu_output_value = val_rs | (instr_ex.immediate & 0xFFFF); break; // Zero extend for logical
            case ANDI: ex_alu_output_value = val_rs & (instr_ex.immediate & 0xFFFF); break;
            case XORI: ex_alu_output_value = val_rs ^ (instr_ex.immediate & 0xFFFF); break;
            // Memory
            case LDW:
                pipeline[EX].branch_target = val_rs + instr_ex.immediate; // Store effective_address
                ex_alu_output_value = 0; // Actual data loaded in MEM stage
                break;
            case STW:
                pipeline[EX].branch_target = val_rs + instr_ex.immediate; // Store effective_address
                ex_alu_output_value = val_rt; // This is the data to store, pass it to MEM via result_val
                break;
            // Control
            case BZ:
                if (val_rs == 0) {
                    pipeline[EX].branch_taken = 1;
                    pipeline[EX].branch_target = pipeline[EX].pc + (instr_ex.immediate * 4);
                }
                break;
            case BEQ:
                if (val_rs == val_rt) {
            ...
            case JR:
                ...
                break;
            default: ex_alu_output_value = 0; break;
        }
        pipeline[EX].result_val = ex_alu_output_value; // Store result for MEM stage / forwarding

        if (pipeline[EX].branch_taken) {
            pipeline_pc = pipeline[EX].branch_target; // Update global fetch PC
            flush_for_branch = 1;                     // Signal to flush IF and ID
            total_flushes += 2;                       // Or however you count flushes
        }
    } else {
        pipeline[EX].result_val = 0; // Ensure result_val is zeroed if EX is invalid/NOP
    }
    ...
    } else if (!stall_for_load_use) { // If not stalling and not flushing, normal advance IF to ID
        pipeline[ID] = pipeline[IF];
        insert_nop(IF, pipeline); // Prepare IF for new fetch (or it gets NOP if fetch fails)
    }
