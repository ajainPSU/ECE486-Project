...
static void simulate_one_cycle_with_forwarding_internal() {
    ...
    if (pipeline[EX].valid && !is_nop(pipeline[EX].instr)) {
        ...
        switch (instr_ex.opcode) {
            // ALU R-Type
            case ADD:  ex_stage_output_value = val_rs + val_rt; break;
            case SUB:  ex_stage_output_value = val_rs - val_rt; break;
            // ... (Include ALL your ALU R-Type and I-Type cases here) ...
            case MUL:  ex_stage_output_value = val_rs * val_rt; break;
            case OR:   ex_stage_output_value = val_rs | val_rt; break;
            case AND:  ex_stage_output_value = val_rs & val_rt; break;
            case XOR:  ex_stage_output_value = val_rs ^ val_rt; break;
            case ADDI: ex_stage_output_value = val_rs + instr_ex.immediate; break;
            case SUBI: ex_stage_output_value = val_rs - instr_ex.immediate; break;
            case MULI: ex_stage_output_value = val_rs * instr_ex.immediate; break;
            case ORI:  ex_stage_output_value = val_rs | (instr_ex.immediate & 0xFFFF); break;
            case ANDI: ex_stage_output_value = val_rs & (instr_ex.immediate & 0xFFFF); break;
            case XORI: ex_stage_output_value = val_rs ^ (instr_ex.immediate & 0xFFFF); break;
